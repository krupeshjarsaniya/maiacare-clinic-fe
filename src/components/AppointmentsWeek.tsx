"use client";
import { AppointmentsType } from "@/utlis/StaticData";
...
interface AppointmentsWeekProps {
  CalnderAppointmentsWeek: AppointmentsType[];
  setBookAppointmentModal: Dispatch<SetStateAction<boolean>>;
  selectedDate: string | null;
  setClickDate: Dispatch<SetStateAction<string>>;
  setClickTime: Dispatch<SetStateAction<string>>;
}
            ...
            <div className="col p-0">
              <div className="row g-0 flex-nowrap aw-days-row text-center">
                {days.map((day, i) => {
                  ...
                  const dateNumber = currentDate
                    ? currentDate.getDate()
                    : i + 1;

                  return (
                    <div className="aw-day-col py-3 border-end" key={day}>
                      <div className={isHighlighted ? "day-text-color " : ""}>
                        {day}
                      </div>
                      <div
                        className={`mt-1 mx-auto rounded-circle d-flex align-items-center justify-content-center aw-date-circle ${
                          isHighlighted
                            ? "aw-date-circle-active"
                            : "text-secondary"
                        }`}
                      >
          ...
          <div className="row aw-body">
            ...
            <div className="col-2 border-end bg-light text-center small text-muted p-0 aw-time-col aw-sticky-col">
              {times.map((time) => (
                <div
                  key={time}
                  className="border-bottom appointments-week-time time-cell"
                >
                  {time}
                </div>
              ))}
                  ...
                  <div className="aw-day-col border-end p-0" key={day}>
                    {times.map((time) => (
                      <OverlayTrigger
                        ...
                        overlay={
                          ...
                          selectedSlot?.time === time ? (
                            <Tooltip
                              id={`tooltip-${day}-${time}`}
                              className="appointments-week-tooltip"
                            >
                              <div className="text-dark small">
                                <p className="mb-1">
                                  Get started by clicking anywhere on the
                                  calendar to add your first appointment
                                </p>
                      ...
                      >
                        ...
                        >
                          {bookedSlots.includes(`${day}|${time}`) && (
                            <div className="aw-booked-line"></div>
                          )}
                          {selectedSlot?.day === day &&
                            selectedSlot?.time === time && (
                              <div className="aw-selected-dot"></div>
                            )}
                        </div>
                      </OverlayTrigger>
                    ))}
